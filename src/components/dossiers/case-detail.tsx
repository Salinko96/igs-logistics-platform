      ...
      >
        <DialogContent className="max-w-lg">
          ...
          <form
            ...
            onSubmit={async (event) => {
              ...
              try {
                ...
                setDocumentForm({ name: '', category: 'autre', fileUrl: '', notes: '' })
                await refetch()
              } catch (error) {
                setDocumentError(error instanceof Error ? error.message : 'Erreur inconnue')
              } finally {
                setDocumentBusy(false)
              }
            }}
          >
            <div className="space-y-2">
              <Label>Nom</Label>
              <Input value={documentForm.name} onChange={(e) => setDocumentForm((s) => ({ ...s, name: e.target.value }))} placeholder="BL, facture, certificat..." />
            </div>
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2">
                <Label>Catégorie</Label>
                <Select value={documentForm.category} onValueChange={(value) => setDocumentForm((s) => ({ ...s, category: value }))}>
              ...
              <div className="space-y-2">
                ...
                <Input value={documentForm.fileUrl} onChange={(e) => setDocumentForm((s) => ({ ...s, fileUrl: e.target.value }))} placeholder="https://..." />
              </div>
            </div>
            <div className="space-y-2">
              <Label>Notes</Label>
              <Textarea value={documentForm.notes} onChange={(e) => setDocumentForm((s) => ({ ...s, notes: e.target.value }))} placeholder="Observations éventuelles..." />
            </div>
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={documentBusy || !documentForm.name.trim()}>
                {documentBusy ? 'Ajout...' : 'Ajouter'}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="grid grid-cols-2 gap-x-4 gap-y-3 text-sm">
                    ...
                    <InfoRow label="Incoterm" value={c.incoterm || '—'} />
