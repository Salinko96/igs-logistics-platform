...
export async function GET(request: NextRequest) {
  try {
    ...
    const [invoices, total, aggregate, overdue] = await Promise.all([
      db.invoice.findMany({ where, include: {
        organization: { select: { name: true } },
        client: { select: { name: true } },
        case: { select: { reference: true } },
      }, orderBy: { createdAt: 'desc' }, skip, take: pageSize }),
      db.invoice.count({ where }),
      db.invoice.aggregate({ where: { ...baseWhere, status: { not: 'annulee' } }, _sum: { netAmount: true, paidAmount: true, amountPayable: true } }),
      db.invoice.count({ where: { ...baseWhere, status: { notIn: ['annulee', 'payee'] }, dueDate: { lt: new Date() } } }),
    ])
...
export async function POST(request: NextRequest) {
  try {
    ...
    const [organization, client] = await Promise.all([
      db.organization.findFirst({ where: { id: profile.organizationId, isActive: true }, select: { id: true, name: true, address: true, city: true, country: true, phone: true, email: true, taxId: true } }),
      db.client.findFirst({ where: { id: clientId, organizationId: profile.organizationId }, select: { id: true } }),
    ])
