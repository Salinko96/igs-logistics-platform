...
export async function GET() {
  ...
  try {
    const [{ subscription, usage }, plans, payments] = await Promise.all([
      getSaaSState(profile.organizationId),
      db.saaSPlan.findMany({ where: { isActive: true }, orderBy: { displayOrder: 'asc' } }),
      db.saaSSubscriptionPayment.findMany({ where: { organizationId: profile.organizationId }, orderBy: { createdAt: 'desc' }, take: 20 }),
    ])
