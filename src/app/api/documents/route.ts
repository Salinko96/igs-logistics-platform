...
export async function GET(request: NextRequest) {
  try {
    ...
    const [documents, total, validated, pending, rejected] = await Promise.all([
      db.document.findMany({ where: whereClause, include: { case: { select: { reference: true } } }, orderBy: { createdAt: 'desc' }, skip, take: pageSize }),
      db.document.count({ where: whereClause }),
      db.document.count({ where: { ...baseWhere, status: { in: ['valide', 'conforme'] } } }),
      db.document.count({ where: { ...baseWhere, status: { in: ['recu', 'en_attente', 'en_verification'] } } }),
      db.document.count({ where: { ...baseWhere, status: { in: ['rejete', 'non_conforme'] } } }),
    ])
