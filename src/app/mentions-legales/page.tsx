...
export default function LegalNoticePage() {
  return <LegalPage title="Mentions légales" description="Identification de l’éditeur, responsabilités et informations relatives à l’hébergement de la plateforme.">
    <LegalNotice>Les champs indiqués « À compléter par l’éditeur » doivent être renseignés et vérifiés avant toute commercialisation externe de la plateforme.</LegalNotice>
    <LegalSection title="Éditeur de la plateforme"><dl className="grid gap-2 sm:grid-cols-[180px_1fr]"><dt>Dénomination</dt><dd className="font-medium">{legalIdentity.publisherName}</dd><dt>Forme juridique</dt><dd>{legalIdentity.legalForm}</dd><dt>Capital social</dt><dd>{legalIdentity.capital}</dd><dt>RCCM</dt><dd>{legalIdentity.rccm}</dd><dt>NIF</dt><dd>{legalIdentity.taxId}</dd><dt>Siège</dt><dd>{legalIdentity.address}</dd><dt>Téléphone</dt><dd>{legalIdentity.phone}</dd><dt>Email</dt><dd><a href={`mailto:${legalIdentity.email}`} className="text-[#14554d] underline">{legalIdentity.email}</a></dd></dl></LegalSection>
