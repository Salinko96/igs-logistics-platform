...
export async function getSaaSState(organizationId: string) {
  ...
  const [cases, users, storage] = await Promise.all([
    db.case.count({ where: { organizationId, createdAt: { gte: start, lt: end } } }),
    db.profile.count({ where: { organizationId, isActive: true } }),
    db.document.aggregate({ where: { organizationId }, _sum: { fileSize: true } }),
  ])
